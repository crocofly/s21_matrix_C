...
START_TEST(s21_sum_matrix_right) {
  int rows = rand() % 100 + 1;
...
START_TEST(s21_sum_matrix_wrong_numb) {
  int rows = rand() % 100 + 1;
  int columns = rand() % 100 + 1;
  matrix_t A = {0};
  matrix_t B = {0};
...
START_TEST(s21_sum_matrix_null) {
  int rows = 0;
  int columns = rand() % 100 + 1;
  matrix_t A = {0};
  matrix_t B = {0};
...
Suite *sum_matrix_suite(void) {
  ...
  // s21_sum_matrix
  tcase_add_test(tc_sum, s21_sum_matrix_right);
  tcase_add_test(tc_sum, s21_sum_matrix_wrong_numb);
  tcase_add_test(tc_sum, s21_sum_matrix_w_r);
  tcase_add_test(tc_sum, s21_sum_matrix_w_c);
  tcase_add_test(tc_sum, s21_sum_matrix_null);
